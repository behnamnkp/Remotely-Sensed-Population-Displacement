...
for mdl in models:
    ...
    y, X = dmatrices(
        "estPop2013_den ~ disNTL2013_hr_den + disNTL2013_nr_den + disNTL2013_lr_den + disNTL2013_bg_den",
    ...
    plt.title('Local Spatial Autocorrelation of residuals for mapping popualtion (2013)')
    plt.show()
    ...
    if mdl == 'nontl':
        ...
        model_NTL_Pop = ols("Pop2013 ~  NTL2013_hr + NTL2013_nr + NTL2013_bg + NTL2013_lr",ntl_scale_NTL2).fit()
        print(model_NTL_Pop.summary())
        print("\nRetrieving manually the parameter estimates:")
        print(model_NTL_Pop._results.params)

        print('Multiple Linear Regression: pop_medntl_annual_incorrected')
        model_NTL_Pop = ols("Pop2013 ~  NTL2013_hr + NTL2013_nr",ntl_scale_NTL2).fit()
        print(model_NTL_Pop.summary())
        print("\nRetrieving manually the parameter estimates:")
        print(model_NTL_Pop._results.params)
        ...
        model_pop_ntl_spatial = ols(
            "Pop2013 ~  NTL2013_hr + NTL2013_nr + clusters2013popntl_HH + clusters2013popntl_HL + clusters2013popntl_LH + clusters2013popntl_NS",
            cluster).fit()
        ...
        model_pop_ntl_spatial_pred = cluster['NTL2013_hr'] * model_pop_ntl_spatial.params[1] + cluster['NTL2013_nr'] * model_pop_ntl_spatial.params[2] +\
                                     cluster['clusters2013popntl_HH'] * model_pop_ntl_spatial.params[3] + \
                                     cluster['clusters2013popntl_HL'] * model_pop_ntl_spatial.params[4] + \
                                     cluster['clusters2013popntl_LH'] * model_pop_ntl_spatial.params[5] + \
                                     cluster['clusters2013popntl_NS'] * model_pop_ntl_spatial.params[6]
...
axs[1,2].get_yaxis().set_visible(False)
axs[1,2].title.set_text('Estimated population (2018)')
plt.suptitle("Population (Landuse-NTL Model)", size=16)
...
axs[1,2].get_yaxis().set_visible(False)
axs[1,2].title.set_text('Estimated population change (2018)')
plt.suptitle("Population Change (Landuse-NTL Model)", size=16)
...
df.iloc[-1, :] = [0.98297, 0.966076 , 0.9820491 , 0.9572736]
...
totalpop.columns = ['Landuse', 'Landuse-NTL']
...
intersect2.plot(column='disPop2013_prime', cmap='Spectral_r', linewidth=0.1, ax=axs, edgecolor='white', legend=True, scheme='quantiles', k=5)
axs.get_xaxis().set_visible(False)
axs.get_yaxis().set_visible(False)
