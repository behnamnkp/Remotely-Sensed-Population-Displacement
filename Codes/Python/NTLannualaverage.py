...
# os.chdir('C:/wgetdown/MosulVNP2/extract2012/brdf')#: e.g. 'G:/sigspatial/HDFSLK/'
...
#         name = 'prj_' + item
#         print(name)
#         arcpy.ProjectRaster_management(in_raster=item, out_raster="C:/wgetdown/MosulVNP2/extract2012/brdf_prj/" + name,
#                                     out_coor_system=sr)
#
# #  2) clip night light data
# os.chdir('C:/wgetdown/MosulVNP2/extract2012/brdf_prj/')#: e.g. 'G:/sigspatial/HDFSLK/'
# # read all file names. Make sure you do not have other file types in the directory
# rasterFiles = os.listdir(os.getcwd())
# for item in rasterFiles:
#     if '.tif' in item and '.xml' not in item and '.ovr' not in item and '.cpg' not in item and '.dbf' not in item :
...
#
#             out_raster="C:/wgetdown/MosulVNP2/extract2012/brdf_prj_clp/" + name)
...
os.chdir('C:/wgetdown/MosulVNP2/extract2012/brdf_prj_clp')  #: e.g. 'G:/sigspatial/HDFSLK/'
# read all file names. Make sure you do not have other file types in the directory
rasterFiles = os.listdir(os.getcwd())
for year in years:
    print('starting the year... ' + year)
...
#     # sns.lineplot(x = range(0, 366), y = ntl_[:, 21, 21], ax=axes[0],color='orange')
#     # axes[0].set(xlabel='Day')
#     # sns.lineplot(x = range(0, 366), y = zenith_[:, 21, 21], ax=axes[0],color='green')
#     # axes[0].set(xlabel='Day')
#     # axes[0].legend(['NTL','VZA'])
#     #
#     # sns.lineplot(x = range(0, 366), y = ntl_[:, 5, 40], ax=axes[1],color='orange')
#     # axes[1].set(xlabel='Day')
#     # sns.lineplot(x = range(0, 366), y = zenith_[:, 5, 40], ax=axes[1],color='green')
#     # axes[1].set(xlabel='Day')
#     # axes[1].legend(['NTL','VZA'])
#     #
#     # sns.lineplot(x = range(0, 366), y = ntl_[:, 38, 26], ax=axes[2],color='orange')
#     # axes[2].set(xlabel='Day')
#     # sns.lineplot(x = range(0, 366), y = zenith_[:, 38, 26], ax=axes[2],color='green')
#     # axes[2].set(xlabel='Day')
#     # axes[2].legend(['NTL','VZA'])
#     #
#     # sns.lineplot(x = range(0, 366), y = ntl_[:, 4, 18], ax=axes[3],color='orange')
#     # axes[3].set(xlabel='Day')
#     # sns.lineplot(x = range(0, 366), y = zenith_[:, 4, 18], ax=axes[3],color='green')
#     # axes[3].set(xlabel='Day')
...
#     zenith_2 = zenith_[:, :, :].reshape(366, 49*44)
#     azimuth_2 = azimuth_[:, :, :].reshape(366, 49*44)
#     ntl_2 = ntl_[:, :, :].reshape(366, 49*44)
#
#     # depth is the ntl, vza, vaa
#     df = np.zeros((366, 49*44, 3))
#     df[:,:,0] = ntl_2
#     df[:,:,1] = zenith_2
#     df[:,:,2] = azimuth_2
#
...
for year in years:
    ...
    out_rasterdataset = 'G:/backupC27152020/Population_Displacement_Final/Resources/VIIRS/VNP46A2/ntl_corrected_med_annually' + year + '.tif'
