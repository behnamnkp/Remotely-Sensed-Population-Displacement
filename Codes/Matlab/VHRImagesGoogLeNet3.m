...
digitDatasetPath = fullfile(...
    'C:\Users\bnikparv\Downloads\VHRClassified\VHRtrainingsamples\train15');
...
%[~,sample] = splitEachLabel(imds,0.9);
%[imdsTrain,imdsValidation] = splitEachLabel(imds,0.7);

train_data = imds;
rng(54321)
[train_data,imdsValidation] = splitEachLabel(train_data,0.8);
...
options = trainingOptions('adam', ...
    ...
    'Verbose',true, ...
    'CheckpointPath','D:\Population_displacement\VHRimages\traincorrections', ...
...
%%
classes = ["BG","MX","RH", "RN"];
load('D:\Population_displacement\VHRimages\traincorrections\net_checkpoint__5022__2020_08_09__20_24_16.mat');

exportONNXNetwork(net,'net_checkpoint__5022__2020_08_09__20_24_16.onnx');

modelfile = 'net_12_8_20.onnx';
classes = ["BG","MX","RH", "RN"];
...
%lb14g = imread('C:\Users\bnikparv\Downloads\VHRClassified\rsm.tif');
lb15g = imread('C:\Users\bnikparv\Downloads\VHRClassified\rsm_15.tif');
lb16g = imread('C:\Users\bnikparv\Downloads\VHRClassified\rsm_16.tif');
lb17g = imread('C:\Users\bnikparv\Downloads\VHRClassified\rsm_17.tif');
lb18g = imread('C:\Users\bnikparv\Downloads\VHRClassified\rsm_18.tif');
...
% end
%%
trajectories = reshape(all, [4242*4951, 4]);
...
plt = imread('vhr_Xception_16.tif');
% georeference images
a = uint8(plt);
%a = imread('lb15gcor0.tif');
% open a file for writing
fid = fopen('vhr_Xception_16.txt', 'wt');
...
% should I separate the patches before executing this code? no
clc;
clear;
load('G:\backupC27152020\C\models2\matlab\trainTraj_4643.mat');

listing = dir(...
    'C:\Users\bnikparv\Downloads\VHRClassified\VHRtrainingsamples\train18_1');
...
for i=3:size(listing, 1)
    ...
    idx_ = strfind(string(d(:, 14)), string(uniquenames(idx2)) + '.TIF');
    idx__ = strfind(string(d(:, 14)), (replace(string(uniquenames(idx2)),'_','') + '.TIF'));
    idx3=0;
    for j=1:size(idx_,1)
       if (cell2mat(idx_(j)) == 1)
           idx3 = j;
       elseif (cell2mat(idx__(j)) == 1)
           idx3 = j;
       end
    end
    label = cell2mat(d(idx3,15));
    
    switch label 
        case 1
            n1= strcat('bg18_1_' + string(idx2));
            n2 = replace(baseFileName, baseFileName2, n1);
            movefile(listing(i).name, n2);

        case 2
            n1= strcat('mx18_1_' + string(idx2));
            n2 = replace(baseFileName, baseFileName2, n1);
            movefile(listing(i).name, n2);

        case 3
            n1= strcat('rh18_1_' + string(idx2));
            n2 = replace(baseFileName, baseFileName2, n1);
            movefile(listing(i).name, n2);
            
        otherwise
            n1= strcat('rn18_1_' + string(idx2));
            n2 = replace(baseFileName, baseFileName2, n1);
            movefile(listing(i).name, n2);
    end
end
...
lb(:,:,1)=imread('C:\Users\bnikparv\Downloads\plnt\results\nov15_cls.tif');
lb(:,:,2)=imread('C:\Users\bnikparv\Downloads\plnt\results\jan16_cls.tif');
lb(:,:,3)=imread('C:\Users\bnikparv\Downloads\plnt\results\feb16_cls.tif');
lb(:,:,4)=imread('C:\Users\bnikparv\Downloads\plnt\results\apr16_cls.tif');
lb(:,:,5)=imread('C:\Users\bnikparv\Downloads\plnt\results\jun16_cls.tif');
lb(:,:,6)=imread('C:\Users\bnikparv\Downloads\plnt\results\jul16_cls.tif');
lb(:,:,7)=imread('C:\Users\bnikparv\Downloads\plnt\results\sep16_cls.tif');
lb(:,:,8)=imread('C:\Users\bnikparv\Downloads\plnt\results\oct16_cls.tif');
lb(:,:,9)=imread('C:\Users\bnikparv\Downloads\plnt\results\nov16_cls.tif');
lb(:,:,10)=imread('C:\Users\bnikparv\Downloads\plnt\results\dec16_cls.tif');
lb(:,:,11)=imread('C:\Users\bnikparv\Downloads\plnt\results\jan17_cls.tif');
...
clear
clc
img(:,:,1) = uint8(imread('C:\Users\bnikparv\Downloads\VHRClassified\rsm_15.tif'));
img(:,:,2) = uint8(imread('C:\Users\bnikparv\Downloads\VHRClassified\rsm_16.tif'));
img(:,:,3) = uint8(imread('C:\Users\bnikparv\Downloads\VHRClassified\rsm_17.tif'));
img(:,:,4) = uint8(imread('C:\Users\bnikparv\Downloads\VHRClassified\rsm_18.tif'));
classNames = [ "No value", "Background","Mixed","High_residential", "Non_residential"]; 

figure
image(img(:,:,4))
cmap = prism(numel(classNames));
...
unique_a = histogram(img(:,:,1))
c(1,:) = unique_a.Values

unique_a = histogram(img(:,:,2))
c(2,:) = unique_a.Values

unique_a = histogram(img(:,:,3))
c(3,:) = unique_a.Values

unique_a = histogram(img(:,:,4))
c(4,:) = unique_a.Values
